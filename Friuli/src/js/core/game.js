...
function gestisciStato3(mezzo, call) {
    ...
    if (call && call.selectedChiamata) {
        ...
        } else if (mezzoType === 'AM') {
            reportKey = 'MSA2'; // AM usa report MSA2
    ...
    if (mezzo.stato === 3 && !mezzo._timerReportPronto && !mezzo._reportProntoInviato) {
        ...
        if (myCall && myCall.mezziAssegnati.length > 1) {
            ...
            function getPriorita(tipoMezzo) {
                ...
                // Priorità 3: MSA2, MSA2_A, MSA, ALS, AM, ELI (massima priorità)
                if (tipoMezzo.startsWith('MSA2') || tipoMezzo === 'MSA2_A' || 
                    tipoMezzo === 'MSA' || tipoMezzo.startsWith('ALS') || 
                    tipoMezzo.startsWith('AM') || tipoMezzo === 'ELI') return 3;
...
class EmergencyDispatchGame {
    ...
    initializeMap() {
        ...
        let initCenter = [45.9045, 13.3097]; // default Palmanova
        const initZoom = 10;
        
        switch (window.selectedCentral) {
            case 'SORES':
                initCenter = [45.9045, 13.3097]; break;
        }
    ...
    async loadMezzi() {
        try {
            // Carica mezzi per Friuli
            const allRaw = [];
            const res = await fetch('src/data/Mezzi FVG.json');
            const data = await res.json();
            (Array.isArray(data)? data : Object.values(data).find(v=>Array.isArray(v))||[])
            .forEach(item => { allRaw.push({ ...item, central: 'SORES' }); });
        ...
        // Carica HEMS
        try {
            const hemsRes = await fetch('src/data/HEMS.json');
            const hemsData = await hemsRes.json();
            (Array.isArray(hemsData)? hemsData : Object.values(hemsData).find(v=>Array.isArray(v))||[])
            .forEach(h => {
                if (true) { // Load all HEMS for Friuli
                    ...
                    if(lat!=null&&lon!=null) {
                        ...
                        const nomePost = mezzo.postazione.trim();
    ...
    async loadHospitals() {
        try {
            // Carica ospedali per Friuli
            const hospitalsAll = [];
            const res = await fetch('src/data/Ospedali FVG.json');
            const data = await res.json();
            (Array.isArray(data)? data : Object.values(data).find(v=>Array.isArray(v))||[])
            .forEach(h => {
    ...
    updatePostazioneMarkers() {
        ...
        Object.values(this.postazioniMap).forEach(postazione => {
            ...
            const isSpecial = postazione.isHems;
            ...
            marker.on('popupopen', () => {
                ...
                const isSpecialUpdate = postazione.isHems;
        ...
        function attachBtnListeners() {
            document.querySelectorAll('.btn-rapido-mezzo').forEach(btn => {
                btn.onclick = function() {
                    ...
                    if (tipo === 'AM') {
                        // Per AM cerchiamo AM
                        mezziTipo = mezziFiltrati.filter(m => m.tipo_mezzo && m.tipo_mezzo === 'AM' && [1,7].includes(m.stato));
                    } else {
