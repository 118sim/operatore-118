...
class EmergencyDispatchGame {
    constructor() {
        ...
        this.missionCounter = { ASL1: 0, ASL2: 0, ASL3: 0, ASL4: 0, ASL5: 0 };
        ...
        const getRate = () => {
            ...
            const lambdaPeak = 1/180;       // peak: 1 call per 180s (ridotto da 60s)
            const lambdaNightPeak = 1/270;  // night peak: 1 call per 270s (ridotto da 90s)
            const lambdaDay = 1/360;        // daytime: 1 call per 360s (ridotto da 120s)
            const lambdaNight = 1/900;      // night: 1 call per 900s (ridotto da 300s)
    ...
    initializeMap() {
        ...
        let initCenter = [45.7375, 7.3201]; // Aosta
        const initZoom = 11;
        switch (window.selectedCentral) {
            case '118 Val d\' Aosta': // Aosta
                initCenter = [45.7375, 7.3201]; break;
        }
    ...
    async loadMezzi() {
         // Carica mezzi dalla centrale Valle d'Aosta
    const allRaw = [];
    try {
        const res = await fetch('src/data/Mezzi.json');
        if (!res.ok) return console.error(`File src/data/Mezzi.json mancante (HTTP ${res.status})`);
        let data = await res.json();
        // Gestisci struttura JSON
        if (data.Sheet1 && Array.isArray(data.Sheet1)) {
            data = data.Sheet1;
        } else if (!Array.isArray(data)) {
            data = Object.values(data).find(v => Array.isArray(v)) || [];
        }
        data.forEach(item => { allRaw.push({ ...item, central: '118 Val d\' Aosta' }); });
    } catch (e) { console.error(`Errore fetch src/data/Mezzi.json`, e); }
    ...
    async loadHospitals() {
        try {           
           // Carica ospedali per Valle d'Aosta
           const hospitalsAll = [];
           const res = await fetch('src/data/Ospedali.json');
        ...
        Object.values(this.postazioniMap).forEach(postazione => {
           ...
           // }
            ...
            const isSpecial = postazione.isCreli ||
                (sel === 'ASL1' && (postazione.isASL2 || postazione.isASL3 || postazione.isASL4 || postazione.isASL5)) ||
                (sel === 'ASL2' && (postazione.isASL1 || postazione.isASL3 || postazione.isASL4 || postazione.isASL5)) ||
                (sel === 'ASL3' && (postazione.isASL1 || postazione.isASL2 || postazione.isASL4 || postazione.isASL5)) ||
                (sel === 'ASL4' && (postazione.isASL1 || postazione.isASL2 || postazione.isASL3 || postazione.isASL5)) ||
                (sel === 'ASL5' && (postazione.isASL1 || postazione.isASL2 || postazione.isASL3 || postazione.isASL4));
    ...
    updateMezzoMarkers() {
        ...
        this.mezzi.forEach(m => {
            ...
            const prefixMap = { ASL1:['ASL2','ASL3','ASL4','ASL5'], ASL2:['ASL1','ASL3','ASL4','ASL5'], ASL3:['ASL1','ASL2','ASL4','ASL5'], ASL4:['ASL1','ASL2','ASL3','ASL5'], ASL5:['ASL1','ASL2','ASL3','ASL4'] };
            ...
            if (!m._marker) {
                ...
                m._marker.setLatLng([m.lat, m.lon]);
                m._marker.setIcon(icon);
                // Update popup content
                m._marker.setPopupContent(`<b>Nome Radio:</b> ${markerName}<br><b>Tipo:</b> ${m.tipo_mezzo || 'N/D'}<br><b>Stato:</b> ${statoDescrizione}`);
            }
        });
    }

    async moveMezzoGradualmente(mezzo, lat1, lon1, lat2, lon2, durataMinuti, statoFinale, callback) {
        // Se non è ELI, prova a usare percorso stradale, altrimenti fallback a linea retta
        let percorso = [[lat1, lon1], [lat2, lon2]];
        if (mezzo.tipo_mezzo !== 'ELI') {
            try {
                // Prova a ottenere percorso stradale, se fallisce usa linea retta
                if (typeof getPercorsoStradaleOSRM === 'function') {
                    percorso = await getPercorsoStradaleOSRM(lat1, lon1, lat2, lon2);
                }
            } catch (error) {
                console.log(`[DEBUG] Errore nel calcolo percorso per ${mezzo.nome_radio}, uso linea retta:`, error);
    ...
    generateNewCall() {
        ...
        const codeMap = { ASL1: 1, ASL2: 2, ASL3: 3, ASL4: 4, ASL5: 5 };
        ...
        function attachBtnListeners() {
            document.querySelectorAll('.btn-rapido-mezzo').forEach(btn => {
                btn.onclick = function() {
                    ...
                    let mezziTipo;
                    if (tipo.includes(',')) {
                        // Gestisci tipi multipli separati da virgola (es. 'MSA,VLV')
                        const tipi = tipo.split(',');
                        mezziTipo = mezziFiltrati.filter(m => m.tipo_mezzo && tipi.some(t => m.tipo_mezzo.startsWith(t.trim())) && [1,7].includes(m.stato));
                    } else {
