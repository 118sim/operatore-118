...
// Override per garantire la cancellazione corretta dei timer simulati
...
class EmergencyDispatchGame {
    constructor() {
        ...
        // Counters for mission numbers per ASL central
        this.missionCounter = { ASL1: 0, ASL2: 0, ASL3: 0, ASL5: 0 };
    ...
    initializeMap() {
        ...
        // Determine initial map center and zoom based on selected ASL central
        let initCenter = [43.8869, 8.0279]; // default ASL1 Imperia
        const initZoom = 11;
        switch (window.selectedCentral) {
            case 'ASL1': // Imperia
                initCenter = [43.8869, 8.0279]; break;
            case 'ASL2': // Ospedale Savona
                initCenter = [44.2978, 8.4673]; break;
            case 'ASL3': // Genova Ospedale San Martino
                initCenter = [44.4056, 8.9567]; break;
            case 'ASL5': // Ospedale La Spezia
                initCenter = [44.0987, 9.8248]; break;
        }
    ...
    async loadMezzi() {
         // Carica mezzi da tutte le centrali ASL in parallelo e assegna campo central
    const centralFiles = { 
        ASL1: 'src/data/Mezzi asl 1.json', 
        ASL2: 'src/data/Mezzi asl 2.json', 
        ASL3: 'src/data/Mezzi asl 3.json', 
        ASL5: 'src/data/Mezzi asl 5.json' 
    };
    const allRaw = [];
    await Promise.all(Object.entries(centralFiles).map(async ([code, path]) => {
        try {
            ...
            // Liguria ha struttura diversa con "Sheet1"
    ...
    async loadHospitals() {
        try {           
           // ASL1: mostra ospedali ASL1 per primi, poi altri
           if (window.selectedCentral === 'ASL1') {
               let hospitalsAll = [];
               // 1) Ospedali asl 1.json (ospedali propri)
               const resAsl1 = await fetch('src/data/Ospedali asl 1.json');
               const asl1List = await resAsl1.json();
               (Array.isArray(asl1List)? asl1List : Object.values(asl1List).find(v=>Array.isArray(v))||[])
               .forEach(h => {
               ...
               // 2) Altri ospedali ASL (con prefisso)
               for (let i = 2; i <= 5; i++) {
                   try {
                       const resOther = await fetch(`src/data/Ospedali asl ${i}.json`);
                       const otherList = await resOther.json();
                       (Array.isArray(otherList)? otherList : Object.values(otherList).find(v=>Array.isArray(v))||[]).forEach(h=>{
                           const coords=(h.COORDINATE||'').split(',').map(s=>Number(s.trim()));
                           const lat=coords[0], lon=coords[1];
                           if(lat!=null&&lon!=null) hospitalsAll.push({ nome: `(ASL${i}) ${h.OSPEDALE?.trim()||''}`, lat, lon, indirizzo: h.INDIRIZZO||'', raw: h });
                       });
                   } catch(e) { console.error(`Error loading Ospedali asl ${i}:`, e); }
               }
               // render markers
           ...
           // Carica tutti gli ospedali di tutte le ASL per Liguria
           const hospitalsAll = [];
           const currentAsl = window.selectedCentral;
           
           // Prima carica ospedali della ASL corrente (senza prefisso)
           try {
               let currentNum = currentAsl?.replace('ASL', '') || '1';
               const resCurrent = await fetch(`src/data/Ospedali asl ${currentNum}.json`);
               const currentList = await resCurrent.json();
               (Array.isArray(currentList)? currentList : Object.values(currentList).find(v=>Array.isArray(v))||[])
               .forEach(h => {
                   const coords = (h.COORDINATE||'').split(',').map(s=>Number(s.trim()));
                   const lat = coords[0], lon = coords[1];
                   if(lat!=null&&lon!=null) hospitalsAll.push({ nome: h.OSPEDALE?.trim()||'', lat, lon, indirizzo: h.INDIRIZZO||'', raw: h });
               });
           } catch(e) { console.error('Error loading current ASL hospitals:', e); }
           
           // Poi carica ospedali delle altre ASL (con prefisso)
           for (let i = 1; i <= 5; i++) {
               const aslCode = `ASL${i}`;
               if (aslCode === currentAsl) continue; // Già caricato sopra
               ...
               try {
                   ...
                   (Array.isArray(otherList)? otherList : Object.values(otherList).find(v=>Array.isArray(v))||[]).forEach(h=>{
                       const coords=(h.COORDINATE||'').split(',').map(s=>Number(s.trim()));
                       const lat=coords[0], lon=coords[1];
                       if(lat!=null&&lon!=null) hospitalsAll.push({ nome: `(${aslCode}) ${h.OSPEDALE?.trim()||''}`, lat, lon, indirizzo: h.INDIRIZZO||'', raw: h });
                   });
               } catch(e) { console.error(`Error loading ${aslCode} hospitals:`, e); }
           }
    ...
    updatePostazioneMarkers() {
        ...
        Object.values(this.postazioniMap).forEach(postazione => {
           ...
           // No central compatibility filters needed for Liguria ASL system
           // if (false) {
               // Removed ASL compatibility filtering
           // }
            ...
            // ASL postazioni from other ASL should use white background: detect flags
            ...
            const isSpecial = postazione.isCreli ||
                (sel === 'ASL1' && (postazione.isASL2 || postazione.isASL3 || postazione.isASL5)) ||
                (sel === 'ASL2' && (postazione.isASL1 || postazione.isASL3 || postazione.isASL5)) ||
                (sel === 'ASL3' && (postazione.isASL1 || postazione.isASL2 || postazione.isASL5)) ||
                (sel === 'ASL5' && (postazione.isASL1 || postazione.isASL2 || postazione.isASL3));
    ...
    updateMezzoMarkers() {
        ...
        this.mezzi.forEach(m => {
            ...
            // All ASL centrals are compatible
           ...
           && m.tipo_mezzo !== 'MSA1_A' && m.tipo_mezzo !== 'MSA2_A') {
            ...
            const prefixMap = { ASL1:['ASL2','ASL3','ASL5'], ASL2:['ASL1','ASL3','ASL5'], ASL3:['ASL1','ASL2','ASL5'], ASL5:['ASL1','ASL2','ASL3'] };
    ...
    generateNewCall() {
        ...
        const unita = year % 10;
        const central = window.selectedCentral || 'ASL1';
        const codeMap = { ASL1: 1, ASL2: 2, ASL3: 3, ASL5: 5 };
